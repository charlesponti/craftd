...
export async function getJobApplicationMetrics(
  ...
  const acceptanceRate =
    offersReceived > 0 ? (offersAccepted / offersReceived) * 100 : 0;
...
function calculateTimeMetrics(apps: ReturnType<typeof extractJobApplications>) {
  ...
  const averageTimeToOffer =
    timeToOfferApps.length > 0
      ? timeToOfferApps.reduce((sum, a) => sum + (a.timeToOffer || 0), 0) /
...
function calculateSalaryMetrics(
  ...
  const averageAccepted =
    acceptedSalaries.length > 0
      ? acceptedSalaries.reduce((sum, s) => sum + s, 0) /
        acceptedSalaries.length
      : 0;
  ...
  const negotiatedApps = apps.filter(
    (a) =>
      a.salaryOffered &&
      a.salaryNegotiated &&
      a.salaryNegotiated > a.salaryOffered
  );
  ...
  const averageNegotiationIncrease =
    negotiatedApps.length > 0
      ? negotiatedApps.reduce((sum, a) => {
          ...
          const increase = calculatePercentageChange(
            a.salaryOffered,
            a.salaryNegotiated
          );
          return sum + increase;
...
function calculateSourceMetrics(
  ...
  const sourceMap = new Map<
    string,
    { count: number; responses: number; offers: number }
  >();
...
export async function getJobApplicationFunnel(
  ...
  const funnel = [
    ...
    { stage: "Response", count: apps.filter((a) => a.responseDate).length },
    {
      ...
      count: apps.filter(
        (a) =>
          ...
          (a.interviewDates &&
            Array.isArray(a.interviewDates) &&
            a.interviewDates.length > 0)
      ).length,
    },
    {
      stage: "Interview",
    ...
    {
      stage: "Offer",
      count: apps.filter((a) => a.status === "OFFER" || a.status === "ACCEPTED")
        .length,
    },
    {
      stage: "Accepted",
...
export async function getApplicationsByTimeframe(
  ...
  return apps.filter(
    (app) => app.applicationDate && new Date(app.applicationDate) >= cutoffDate
  );
}
...
export async function getTopCompaniesAppliedTo(userId: string, limit = 10) {
  ...
  const companyMap = new Map<
    string,
    { count: number; offers: number; interviews: number }
  >();
...
export async function getAverageApplicationCycleTime(
  userId: string
): Promise<number> {
  const applicationsResult = await getUserJobApplications(userId);
  const apps = extractJobApplications(applicationsResult);

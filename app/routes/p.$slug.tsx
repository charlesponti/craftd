...
export const meta: MetaFunction<typeof loader> = ({ data }) => {
  ...
  return [
    { title: `${data.portfolio.personalInfo.name} - ${data.portfolio.personalInfo.title}` },
    { name: 'description', content: data.portfolio.personalInfo.bio },
    {
      property: 'og:title',
      content: `${data.portfolio.personalInfo.name} - ${data.portfolio.personalInfo.title}`,
    },
    { property: 'og:description', content: data.portfolio.personalInfo.bio },
    ...
    { name: 'twitter:description', content: data.portfolio.personalInfo.bio },
  ]
}
...
export async function loader({ params }: LoaderFunctionArgs) {
  ...
  // Mock portfolio data - this would fetch from your API
  const portfolio = {
    slug,
    personalInfo: {
      name: 'Sarah Chen',
      title: 'Senior Product Designer',
      bio: 'I design digital experiences that bridge the gap between user needs and business objectives, with a focus on accessibility and sustainable design practices.',
      location: {
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-12">
      ...
      <header className="mb-16">
        <div className="flex items-start justify-between mb-8">
          <div>
            ...
            <p className="text-xl text-gray-600 mb-4">{portfolio.personalInfo.title}</p>
            <p className="text-gray-600 leading-relaxed">{portfolio.personalInfo.bio}</p>
          </div>
        </div>

        {/* Stats */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
          {portfolio.personalInfo.stats.map((stat) => (
            <div key={stat.label} className="text-center">
              <div className="text-2xl font-serif font-light text-gray-900 mb-1">{stat.value}</div>
        ...
        <div className="flex flex-wrap gap-4 text-sm text-gray-600">
          <span>{portfolio.personalInfo.location.current}</span>
          <span>•</span>
          <span>{portfolio.personalInfo.contact.email}</span>
          <span>•</span>
          <span className="text-green-600 font-medium">
            {portfolio.personalInfo.availability.message}
          </span>
        </div>
      </header>
      ...
      <section className="mb-16">
        ...
        <p className="text-gray-700 leading-relaxed">{portfolio.personalInfo.bio}</p>
      </section>
      ...
      <section className="mb-16">
        ...
        <div className="space-y-12">
          {portfolio.workExperience.map((job) => (
            <div key={`${job.title}-${job.role}`} className="border-l-2 border-gray-100 pl-6">

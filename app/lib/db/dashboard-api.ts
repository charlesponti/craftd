...
export async function getDashboardMetrics(
  ...
  try {
    ...
    return {
      currentSalary: `$${centsToDollars(
        financial.currentSalary
      ).toLocaleString()}`,
      ...
      negotiationSuccess: `${jobApplications.salaryMetrics.negotiationSuccessRate.toFixed(
        1
      )}%`,
  ...
  try {
    ...
    const conversionFunnel = [
      { stage: "Applications", count: metrics.totalApplications, rate: 100 },
      {
        stage: "Responses",
        count: Math.round(
          (metrics.totalApplications * metrics.responseRate) / 100
        ),
    ...
    return {
      ...
      salaryMetrics: {
        ...
        averageNegotiationIncrease:
          metrics.salaryMetrics.averageNegotiationIncrease,
      },
    };
  } catch (error) {
